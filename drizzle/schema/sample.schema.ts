import { pgEnum, text, timestamp, index, boolean, decimal, pgTable } from 'drizzle-orm/pg-core';
...
export const categories = pgTable('category', {
  ...
  billboardId: text('billboardId').notNull().references(() => billboards.id),
  name: text('name').notNull(),
  createdAt: timestamp('createdAt').default(sql`now()`).notNull(),
  updatedAt: timestamp('updatedAt').default(sql`now()`).notNull(),
...
export const products = pgTable('product', {
  ...
  sizeId: text('sizeId').notNull().references(() => sizes.id),
  colorId: text('colorId').notNull().references(() => colors.id),
  createdAt: timestamp('createdAt').default(sql`now()`).notNull(),
  updatedAt: timestamp('updatedAt').default(sql`now()`).notNull(),
