      ...
      <SidebarMenu>
        {items.map((item) => {
          ...
          const isAnySubItemActive =
            hasSubItems && item.items?.some((subItem) => isRouteActive(subItem.url));
          ...
          return (
            <Collapsible
              key={item.title}
              asChild
              defaultOpen={isActive || isAnySubItemActive}
                ...
                <CollapsibleTrigger asChild>
                  <SidebarMenuButton tooltip={item.title}>
                    {item.icon && (
                      <item.icon
                        className={cn(
                          isActive || isAnySubItemActive ? "text-primary" : "text-muted-foreground",
                        )}
                      />
                    )}
                    <span>{item.title}</span>
